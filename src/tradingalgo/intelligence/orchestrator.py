...
class IntelligenceOrchestrator:
    def __init__(self, health: ProviderHealthRegistry | None = None,
                 store: SQLiteEvidenceStore | None = None,
                 history: AnalyticalHistory | None = None) -> None:
    ...
    def collect(self, ticker: str, *, quote: list[FetchCandidate] | None = None,
        ...
        for channel, candidates in (("quote", quote), ("analyst", analyst), ("news", news),
                                    ("sec", sec), ("fred", fred), ("events", events)):
            if not candidates:
                continue
            ...
            key = provider if channel == "quote" else (
                "sec_edgar_companyfacts" if channel == "sec" else
                "fred" if channel == "fred" else f"{provider}_{channel}")
        ...
        if isinstance(event_payload, list):
            rows = [row for row in event_payload if isinstance(row, dict)]
        ...
        for row in rows:
            ...
            if exposure:
                evidence.extend(exposure.propagate_event(ticker, title, text, source,
                                                         row.get("url") or row.get("source_url")))
            else:
                evidence.append(event_to_evidence(ticker, title, text, source,
                                                  row.get("url") or row.get("source_url")))
        ...
        if self.history:
            self.history.upsert(evidence)

        point_in_time = as_of or datetime.now(timezone.utc)
        signals = evidence_signals(ticker, evidence)
        signals.extend(build_fundamental_factors(ticker, evidence, as_of=point_in_time, horizon=horizon))
        ...
        return OrchestrationResult(ticker=ticker.upper(), evidence=tuple(evidence), signals=tuple(signals),
            bundle=bundle, advisory=advisory, provider_errors=errors, selected_providers=selected)
