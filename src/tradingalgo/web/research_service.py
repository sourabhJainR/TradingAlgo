...
def analyze_stock(ticker: str, market: str, horizon: str, config: SourceConfig | None = None) -> StockAnalysis:
    ...
    return StockAnalysis(
        ...
        hypothesis=hypothesis, prediction_basis=basis, favorable_market=market_view,
        sector_news=sector_news, risks=risks, data_sources=sources, warnings=warnings,
    )
...
def _nse_quote(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"price": info.get("lastPrice"), "volume": payload.get("securityWiseDP", {}).get("tradedVolume") if isinstance(payload.get("securityWiseDP"), dict) else None}
...
def _technical_metrics(candles: list[Any]) -> dict[str, float]:
    ...
    return {"price": price, "sma20": sma20, "sma50": sma50, "sma200": sma200,
            "atr": atr, "high20": max(highs[-20:]), "low20": min(lows[-20:]),
            "high52": max(highs[-252:]), "low52": min(lows[-252:])}
...
def _levels(price: float | None, m: dict[str, float], horizon: str) -> tuple[float, float, float, float, float] | None:
    if price is None or not m: return None
    atr = max(m["atr"], price * 0.01)
    if horizon == "short":
        center = max(m["sma20"], m["low20"]); low = min(price, center + 0.25 * atr); high = max(price, center + 0.75 * atr)
        stop = max(0.01, low - 1.5 * atr); t1 = high + 2.0 * atr; t2 = high + 4.0 * atr
    else:
        center = max(m["sma50"], m["sma200"]); low = min(price, center * 1.02); high = max(price, center * 1.08)
        stop = max(0.01, min(low - 2.0 * atr, m["sma200"] * 0.90)); t1 = high * 1.20; t2 = high * 1.35
    return low, high, stop, t1, t2


def _market_view(m: dict[str, float]) -> str:
    if not m: return "Unknown: benchmark data is not available"
    p = m["price"]
    if p > m["sma20"] > m["sma50"]: return "Favorable for risk-on setups based on the stock trend"
    if p < m["sma20"] < m["sma50"]: return "Unfavorable: trend is weak and risk is elevated"
...
def _basis(m: dict[str, float], horizon: str, market_view: str, news: list[str]) -> list[str]:
    if not m: return ["Insufficient market-price history; no technical prediction is asserted."]
    return [f"Price versus 20/50/200-session moving averages: {m['price']:.2f} / {m['sma20']:.2f} / {m['sma50']:.2f} / {m['sma200']:.2f}.", f"ATR-based risk distance is {m['atr']:.2f}.", f"20-session range is {m['low20']:.2f} to {m['high20']:.2f}.", f"52-week range is {m['low52']:.2f} to {m['high52']:.2f}.", market_view, f"Horizon rule: {horizon} setup; levels are derived from trend, range and ATR rather than a discretionary price guess."]


def _hypothesis(action: str, m: dict[str, float], horizon: str) -> str:
    if not m: return "The hypothesis cannot be established until sufficient market data is available."
    if action == "BUY": return f"The {horizon} hypothesis is that trend continuation is more likely while price holds above the key moving-average and volatility support levels."
    if action == "AVOID": return f"The {horizon} hypothesis is that downside or failed rebounds remain more likely while price stays below the key trend averages."
...
def _risks(m: dict[str, float], market_view: str, news: list[dict[str, Any]], warnings: list[str]) -> list[str]:
    risks = ["Technical levels can fail after earnings, macro shocks or gap moves.", "Stop-loss levels are research levels, not guaranteed execution prices."]
    if m and m["atr"] > m["price"] * 0.04: risks.append("Recent volatility is high relative to price; position sizing should be conservative.")
    if "Unfavorable" in market_view: risks.append("The current trend context is unfavorable for long exposure.")
    if news: risks.append("News sentiment can change quickly and headlines may be incomplete.")
    if warnings: risks.append("Some requested data sources were unavailable, reducing confidence.")
    return risks
