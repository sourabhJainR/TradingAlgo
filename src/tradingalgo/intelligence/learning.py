...
HORIZON_DAYS: dict[Horizon, int] = {Horizon.INTRADAY: 1, Horizon.SWING: 10, Horizon.MEDIUM: 60, Horizon.LONG: 252}
...
class PredictionSnapshot:
    ...
    def from_advisory(cls, advisory: Advisory, as_of: datetime, prediction_id: str) -> "PredictionSnapshot":
        ...
        return cls(prediction_id, advisory.ticker.upper(), timestamp, advisory.action, advisory.score, advisory.confidence,
                   advisory.horizon, tuple(dict.fromkeys(e for s in advisory.signals for e in s.evidence_ids)), factors)

def prediction_id_for(advisory: Advisory, as_of: datetime) -> str:
    payload = {"ticker": advisory.ticker.upper(), "as_of": _utc(as_of).isoformat(), "action": advisory.action,
               "score": advisory.score, "confidence": advisory.confidence, "horizon": advisory.horizon.value,
...
class LearningStore:
    ...
    def save_prediction(self, snapshot: PredictionSnapshot) -> None:
        self.db.execute("INSERT OR REPLACE INTO predictions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (snapshot.prediction_id, snapshot.ticker, _utc(snapshot.as_of).isoformat(), snapshot.action, snapshot.score,
             snapshot.confidence, snapshot.horizon.value, json.dumps(snapshot.evidence_ids), json.dumps(dict(snapshot.factor_scores), sort_keys=True)))
        self.db.commit()
    ...
    def save_outcome(self, outcome: OutcomeObservation) -> None:
        ...
        self.db.execute("INSERT OR REPLACE INTO outcomes VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (outcome.prediction_id, outcome.ticker.upper(), _utc(outcome.observed_at).isoformat(), outcome.realized_return,
             outcome.benchmark_return, outcome.max_drawdown, outcome.maximum_adverse_excursion, outcome.maximum_favorable_excursion))
        self.db.commit()
...
class FactorCalibrator:
    ...
    def __init__(self, min_weight: float = .50, max_weight: float = 1.50, strength: float = .50, min_observations: int = 5) -> None:
...
def apply_factor_weights(snapshot: PredictionSnapshot, weights: Mapping[str, float]) -> PredictionSnapshot:
    ...
    return PredictionSnapshot(snapshot.prediction_id, snapshot.ticker, snapshot.as_of, snapshot.action, score, snapshot.confidence,
                              snapshot.horizon, snapshot.evidence_ids, adjusted)
...
def statistical_tests(pairs: Sequence[tuple[PredictionSnapshot, OutcomeObservation]], *, samples: int = 1000, seed: int = 42) -> StatisticalTestResult:
    values = [_strategy_excess(s, o) for s, o in pairs]
    n = len(values)
    if not n:
    ...
    for _ in range(sample_count):
        ...
        permuted = [value * sign for value, sign in zip(values, signs)]
        if abs(mean(permuted)) >= abs(observed):
            exceed += 1
...
def baseline_comparison(pairs: Sequence[tuple[PredictionSnapshot, OutcomeObservation]]) -> dict[str, float]:
    ...
    system = [_strategy_excess(s, o) for s, o in pairs]
...
def _metrics(pairs: Sequence[tuple[PredictionSnapshot, OutcomeObservation]], weights: Mapping[str, float]) -> ValidationResult:
    ...
    correct = 0
    brier: list[float] = []
    returns = [_strategy_excess(s, o) for s, o in pairs]
    excess = returns
    ...
    for s, o in pairs:
        ...
        by_action.setdefault(s.action, []).append(_strategy_excess(s, o))
